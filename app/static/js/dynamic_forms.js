    ...
    _createClass(DynamicForms, [{
        ...
        value: function createNewRow(parent, element) {
            ...
            cloned.find("[data-dynamic-form-input-id-template]").each(function (inputKey, inputValue) {
                ...
                var html_id = inputElement.attr('id');
                isFirst = this.dynamic_elements[html_id] === undefined;
                var id = inputElement.data('dynamic-form-input-id-template');

                if (parsedValue !== undefined) {
                    var dynamicID = inputElement.data('dynamic-form-input-name');
                    if (typeof parsedValue === 'string') {
                        inputElement.val(parsedValue);
                    } else if ((typeof parsedValue === 'undefined' ? 'undefined' : _typeof(parsedValue)) === 'object' && parsedValue.hasOwnProperty(dynamicID)) {
                        inputElement.val(parsedValue[dynamicID]);
                    }
                }

                var inputID = void 0;
                if (index === undefined) {
                    if (this.dynamic_elements[html_id] === undefined) {
                        this.dynamic_elements[html_id] = 0;
                    } else {
                        this.dynamic_elements[html_id]++;
                    }

                    inputID = html_id.replace(id, DynamicForms.numToChar(this.dynamic_elements[html_id]));
                } else {
                    inputID = html_id.replace(id, index);
                }

                inputElement.attr('id', inputID);

                // Add attr name, unique for each form el created from template

                var html_name = inputElement.attr('name');
                var inputName = void 0;
                if (index === undefined) {
                    inputName = html_name.replace(id, DynamicForms.numToChar(this.dynamic_elements[html_id]));
                } else {
                    inputName = html_name.replace(id, index);
                }
